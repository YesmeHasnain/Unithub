...
<head>
    ...
    <title>@yield('title','Jampack - Admin CRM Dashboard Template')</title>
    ...
    {{-- avatar images ko hamesha container ke andar fit aur crop karne ke liye --}}
    <style>
        .avatar-img {
            width: 100%;
            height: 100%;
        ...
        /* agar kahin large profile image use karni ho (jaise edit profile page pe) */
        .profile-avatar-lg {
            width: 160px;
            height: 160px;
            object-fit: cover;
...
@php
    ...
    $profilePhotoUrl = $authUser && $authUser->profile_photo_path
        ...
        : asset('dist/img/avatar12.jpg');

    $profileName  = $authUser?->full_name ?? $authUser?->name ?? 'User';
					...
					<li class="nav-item">
						<div class="dropdown dropdown-notifications">
							<a href="#" class="btn btn-icon btn-rounded btn-flush-dark flush-soft-hover dropdown-toggle no-caret"
                               data-bs-toggle="dropdown" data-dropdown-animation role="button" aria-haspopup="true" aria-expanded="false">
								<span class="icon">
									<span class="position-relative">
					...
					<li class="nav-item">
						<div class="dropdown ps-2">
							<a class=" dropdown-toggle no-caret" href="#" role="button" data-bs-display="static"
                               data-bs-toggle="dropdown" data-dropdown-animation data-bs-auto-close="outside" aria-expanded="false">
								<div class="avatar avatar-rounded avatar-xs">
									<img src="{{ $profilePhotoUrl }}" alt="{{ $profileName }}" class="avatar-img">
								</div>
							</a>
							<div class="dropdown-menu dropdown-menu-end">
									...
									<div class="media">
										<div class="media-head me-2">
											<div class="avatar avatar-primary avatar-sm avatar-rounded">
												<span class="initial-wrap">{{ strtoupper(substr($profileName,0,2)) }}</span>
											</div>
										</div>
										<div class="media-body">
											<div class="dropdown">
												<span class="d-block link-dark fw-medium">{{ $profileName }}</span>
											</div>
											<div class="fs-7">{{ $profileEmail }}</div>
											<form method="POST" action="{{ route('logout') }}" class="mt-1">
												@csrf
												<button type="submit" class="d-block fs-8 link-secondary btn btn-link p-0 text-start">
													<u>Sign Out</u>
												</button>
											</form>
										</div>
								...
								<a class="dropdown-item" href="#">
                                    <span class="dropdown-icon feather-icon"><i data-feather="credit-card"></i></span>
                                    <span>Payment methods</span>
                                </a>
								<a class="dropdown-item" href="#">
                                    <span class="dropdown-icon feather-icon"><i data-feather="check-square"></i></span>
                                    <span>Subscriptions</span>
                                </a>
								<a class="dropdown-item" href="#">
                                    <span class="dropdown-icon feather-icon"><i data-feather="settings"></i></span>
                                    <span>Settings</span>
                                </a>
								<div class="dropdown-divider"></div>
								<a class="dropdown-item" href="#">
                                    <span class="dropdown-icon feather-icon"><i data-feather="tag"></i></span>
